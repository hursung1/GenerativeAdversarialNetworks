...
class Generator_FC(torch.nn.Module):
    ...
    def __init__(self, input_node_size, output_shape, hidden_node_size=256,  
        ...
        output_node_size = output_shape[0] * output_shape[1]

        HiddenLayerModule = []
        for i in range(hidden_node_num):
            HiddenLayerModule.append(torch.nn.Linear(hidden_node_size * (i+1), hidden_node_size * (i+2)))
            if normalize:
                HiddenLayerModule.append(torch.nn.BatchNorm1d(hidden_node_size * (i+2), 0.8))
        ...
        self.network = torch.nn.Sequential(
            ...
            *HiddenLayerModule,
            ...
            torch.nn.Linear(hidden_node_size * (hidden_node_num+1), output_node_size),
            torch.nn.Tanh()
...
class Discriminator_FC(torch.nn.Module):
    ...
    """
    def __init__(self, input_shape, hidden_node_size=512, 
                        output_node_size=1, ):
        super(Discriminator_FC, self).__init__()
        input_node_size = input_shape[0] * input_shape[1]
